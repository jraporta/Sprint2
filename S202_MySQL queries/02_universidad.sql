...
SELECT p.id, p.nombre, p.apellido1, p.apellido2, COUNT(a.id) AS n_de_asignaturas FROM profesor pr JOIN persona p ON pr.id_profesor = p.id LEFT JOIN asignatura a ON pr.id_profesor = a.id_profesor GROUP BY p.id ORDER BY n_de_asignaturas desc;
